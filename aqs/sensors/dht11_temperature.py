...
class DHT11TemperatureSensor(Sensor):
    def __init__(self, name, data_pin=board.D4):
        super().__init__(name)
        self._sensor = get_dht11_sensor(data_pin)

    def get_units(self):
        return MeasurementUnit.TEMP_CELCIUM

    def get_readings(self):
        readings = self._sensor.temperature
        LOGGER.debug(f"[DHT11-temp] [{self.get_name()}] get readings: {readings}")
        return readings
